fn food_production_two(data: Vec<Vec<&str>>, seeds: &str) -> i64 {
    ...
    */
    ...
    // make a range of seeds
    let mut available_seeds: Vec<Vec<i64>> = vec![];

    let split_seeds: Vec<&str> = seeds.split(" ").collect();
    let mut prev_seed: i64 = 0;

    for (s_idx, spli_seed) in split_seeds.iter().enumerate() {
        if s_idx % 2 != 0 {
            let top_range = spli_seed.parse::<i64>().unwrap();

            available_seeds.push(vec![prev_seed, prev_seed + top_range]);
        }
        prev_seed = spli_seed.parse::<i64>().unwrap();
    ...
    for ranges in available_seeds.clone() {
        // Use the sqrt search to find the lowest zone
        let jump = ranges[1] - ranges[0];
        let sqrt = (jump as f64).sqrt() as i64;

        let mut locations = vec![];
        let mut idx = ranges[0];
        while idx <= ranges[1] {
            let location = find_final_location(idx, &range_finder, &destination_finder);
        ...
        aggregate_min_locations.push(*min_location);
    }

    // find the min of the aggregate seeds
    let mut lowest = [-1, -1];
    for (i, min) in aggregate_min_locations.iter().enumerate() {
        if lowest[0] == -1 || lowest[0] > *min {
            lowest[1] = i as i64;
            lowest[0] = *min;
        }
    }

    let mut final_start_idx = -1;
    // Top value for range that contains lowest outcome - bottome value of range that contins
    // lowest outcome
    let jump = available_seeds[lowest[1] as usize][1] - available_seeds[lowest[1] as usize][0];
    let sqrt = (jump as f64).sqrt() as i64;

    let mut final_idx = available_seeds[lowest[1] as usize][0];
    let mut compare_value = -1;

    while final_idx <= available_seeds[lowest[1] as usize][1] {
        let location = find_final_location(final_idx, &range_finder, &destination_finder);
        if compare_value == -1 || location < compare_value {
            compare_value = location;
            final_start_idx = final_idx;
        }

        final_idx += sqrt;
    }
    let mut final_location = -1;
    for idx in final_start_idx - sqrt..final_start_idx + sqrt {
        let location = find_final_location(idx, &range_finder, &destination_finder);
        if final_location == -1 || location < final_location {
...
fn find_final_location(
    mut seed: i64,
    range_finder: &Vec<Vec<Vec<i64>>>,
    destination_finder: &Vec<Vec<i64>>,
) -> i64 {
    for (range_i, range) in range_finder.iter().enumerate() {
        for (match_i, potential_match) in range.iter().enumerate() {
            if seed >= potential_match[0] && seed < potential_match[1] {
...
mod tests {
    ...
    fn test_food_production_two() {
        ...
        let seeds ="4088478806 114805397 289354458 164506173 1415635989 166087295 1652880954 340945548 3561206012 483360452 35205517 252097746 1117825174 279314434 3227452369 145640027 2160384960 149488635 2637152665 236791935";
        let result = food_production_two(test_data, seeds);

        assert_eq!(result, 17729182);
    }
}
